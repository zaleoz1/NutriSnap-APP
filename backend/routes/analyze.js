...
roteador.post('/', requerAutenticacao, async (req, res) => {
  ...
  try {
    ...
    const prompt = "Analise esta imagem de uma refeição. Identifique cada item e estime calorias. Responda com JSON: {itens:[{nome, calorias}], caloriasTotais}";
    const payload = {
    ...
    let dados;
    try { dados = JSON.parse(limpo); } catch { dados = null; }
